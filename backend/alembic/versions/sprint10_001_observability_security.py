...
def upgrade() -> None:
    ...
    op.add_column(
        "users",
        sa.Column("permission_scopes", sa.JSON(), server_default=sa.text("'[]'::json"), nullable=False),
        schema="auth",
    )
    ...
    op.create_index(op.f("ix_auth_login_history_success"), "login_history", ["success"], unique=False, schema="auth")
    op.create_index(op.f("ix_auth_login_history_timestamp"), "login_history", ["timestamp"], unique=False, schema="auth")
    ...
    op.create_index(op.f("ix_security_api_secrets_user_id"), "api_secrets", ["user_id"], unique=False, schema="security")
    op.create_index(op.f("ix_security_api_secrets_provider"), "api_secrets", ["provider"], unique=False, schema="security")


def downgrade() -> None:
    op.drop_index(op.f("ix_security_api_secrets_provider"), table_name="api_secrets", schema="security")
    op.drop_index(op.f("ix_security_api_secrets_user_id"), table_name="api_secrets", schema="security")
    op.drop_table("api_secrets", schema="security")

    op.drop_index(op.f("ix_auth_login_history_timestamp"), table_name="login_history", schema="auth")
